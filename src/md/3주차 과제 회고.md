...
### - css 스타일 문제 분석 및 해결 과정

문제점: 위에서도 얘기를 했었지만, 이미지와 배경 이미지에 동적인 효과를 적용하는 부분에 있어서 어려움을 겪었다. 무엇이 원인인지 몰라서 결국 ai에게 물어보니 마크업을 이미지와 배경 이미지를 분리해서 만들어야 된다고 하였다.

```
<html 구조>(수정 전)
<section class="image-container">
            <a href="/" class="game-container game-link5">
                <img class="game-5" src="./images/game-5-img.png" alt="" />
              <h3>롬: 리멤버 오브 마제스티</h3>
              <p>세계는 하나의 전장이 된다</p>
              <p class="game-type">MMORPG</p>
            </a>
          </section>

```

원래 이러한 구조였는데, ai가 이미지 태그에 <div>으로 배경이미지라는 클래스네임으로 한번 더 감싸줘서 여기에 배경 이미지를 추가하는 식으로 구현하라고 하였다.
...
css에서 이제 분리된 구조를 통하여 bg-card에 배경 이미지를 삽입하고, img태그에는 캐릭터 이미지를 넣었다.
그리고 나서 마우스를 댔을때 확대되는 듯한 효과를 주기 위해 transform 과 transition을 같이 사용하였다.

#### - css에서 문제점: 배경이미지와 캐릭터 이미지 바닥 지점이 일치하지 않음!

: 이부분도 어떻게 해결해야 될지 모르겠어서.. 결국 ai의 도움을 받았다. ai는 display: flex와 alignitems:felx-end를 사용하여 해결하라고 했다.

- 의문점: 반복적인 코드 적용

```
bg-card-1 {
  background-image: url(../../images/game-1-bg.png);
  border-radius: 8px;
  height: 160px;
  display: flex;
...
.bg-card-2 {
  background-image: url(../../images/game-2-bg.png);
  border-radius: 8px;
  height: 160px;
  display: flex;
  align-items: flex-end;
...
.bg-card-3 {
  background-image: url(../../images/game-3-bg.png);
  border-radius: 8px;
  height: 160px;
  display: flex;
  align-items: flex-end;
...
.bg-card-4 {
  background-image: url(../../images/game-4-bg.png);
  border-radius: 8px;
  height: 160px;
  display: flex;
  align-items: flex-end;
...
.bg-card-5 {
  /* background-image: url(../../images/game-5-bg.png); */
  border-radius: 8px;
  height: 160px;
  display: flex;
  align-items: flex-end;
...
.bg-card-6 {
  /* background-image: url(../../images/game-6-bg.png); */
  border-radius: 8px;
  height: 160px;
  display: flex;
  align-items: flex-end;
...
이렇게 배경이미지가 다를 뿐이지 적용할 스타일링이 동일할 때 좀 더 효율적으로 스타일 구성을 짤 수 없을지에 대한 의문이 들었다. 그리고 이렇게 이미지에 대한 문제점이 있을때, 보통 이런식으로 개발자들이 해결을 하는지, ai가 맞는 방향으로 알려줬는지에 대한 의문점이 남았다.
궁극적으로 html구조가 잘못인지, css 스타일링을 할 때 내가 놓친 부분이 있었던 건지 이 꼬였던 문제의 시작점이 어딘지 감을 잡을 수 없어서 그 부분이 답답하다.
